    ...
    <Form class="formClass" ref="obj" :model="obj" :label-width="100" :rules="ruleInline">
      <div>
        ...
        <FormItem label="排序码:" prop="sort">
          <Input type="text"  v-model="obj.sort" placeholder="请输入序号">
          </Input>
        </FormItem>
      </div>
      <div>
        <FormItem label="所属分类:" prop="fatherId">
          <Select v-model="obj.fatherId" :label-in-value="true" placeholder="请选择所属分类" @on-change="onChangeFatherCategory">
  ...
  export default {
    ...
    methods: {
      onChangeFatherCategory(item){
        this.$emit('onChangeFatherCategory',item)
      },
    },
    created() {
