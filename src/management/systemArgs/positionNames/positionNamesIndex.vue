...
<script>
  import { positionNamesGetByPage,positionNamesInsert,positionNamesUpdate,positionNamesDelete } from '@/axios/api';
  import tableTemplate from '@/commonComponent/tableTemplate'
  import businessForm from './positionNamesForm'
  export default {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            width: 90,
            // render: (h, params) => {
            //   return h('div',this.$Filters.enableType(params.row.useStatus))
            // }
            render: (h, params) => {
              const row = params.row;
              const str = this.$Filters.enableType(params.row.useStatus)
    ...
    methods: {
      ...
      addBusiness(){
        ...
        this.addBusinessUser.useStatus = 0
        this.addBusinessTitle = '新建 / 职位信息'
      ...
      postDepartments(){
        if(this.addBusinessUser.id){
          positionNamesUpdate({
            ...
            useStatus: this.addBusinessUser.useStatus,
            sort: this.addBusinessUser.sort,
            status: this.addBusinessUser.status,
            remark: this.addBusinessUser.remark,
            updateUser:this.user.user.id,
